...
f(x,n)
```
Now to check if our function is correct, we have an intrinsic functions or built-in functions in R to calculate the mean and the variance for data set. So,
```{r, comment=NA}
mean(x);var(x)
